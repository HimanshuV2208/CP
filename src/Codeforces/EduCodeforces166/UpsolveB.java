...
public class UpsolveB {
    ...
    static void solve() {
        ...
        int extra = b[n];
        long ans = 0;
        int extraSteps = P_INF;
        for(int i = 0; i < n; i++){
            int diff = abs(a[i] - b[i]);
            ans += diff;
            int smaller = min(a[i], b[i]);
            int larger = max(a[i], b[i]);
            if(extra < smaller){
                extraSteps = min(extraSteps, 1 + abs(smaller - extra));
            } else if(extra <= larger){
                extraSteps = 1;
            } else {
                extraSteps = min(extraSteps, 1 + abs(extra - larger));
            }
        }
        System.out.println(ans + extraSteps);
    }

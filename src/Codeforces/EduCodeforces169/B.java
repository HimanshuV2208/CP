...
public class B {
    ...
    static void solve() {
        ...
        IntStream.rangeClosed(a[0], a[1]).forEach(i -> map.put(i, map.getOrDefault(i, 0) + 1));
        IntStream.rangeClosed(b[0], b[1]).forEach(i -> map.put(i, map.getOrDefault(i, 0) + 1));
        int l = -1;
        int r = -1;
        int i = 1;
        while (true) {
        ...
        if (min(a[0], min(a[1], min(b[0], b[1]))) < l) {
            ans++;
        }
        if (max(a[0], max(a[1], max(b[0], b[1]))) > r) {
            ans++;
        }

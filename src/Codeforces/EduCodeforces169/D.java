package Codeforces.EduCodeforces169;
...
public class D {
    ...
    static void solve() {
        int n = sc.nextInt();
        int q = sc.nextInt();
        UnionFind ufBlue = new UnionFind(n + 1);
        UnionFind ufGreen = new UnionFind(n + 1);
        UnionFind ufRed = new UnionFind(n + 1);
        UnionFind ufYellow = new UnionFind(n + 1);
        for (int i = 0; i < n; i++) {
            String str = sc.nextLine();
            if(str.contains("B")) ufBlue.unionByRank(0, i + 1);
            if(str.contains("G")) ufGreen.unionByRank(0, i + 1);
            if(str.contains("R")) ufRed.unionByRank(0, i + 1);
            if(str.contains("Y")) ufYellow.unionByRank(0, i + 1);
        }
        for (int i = 0; i < q; i++) {
            int x = sc.nextInt();
            int y = sc.nextInt();

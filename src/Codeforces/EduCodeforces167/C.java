package Codeforces.EduCodeforces167;
...
public class C {
    ...
    static void solve() {
        int n = sc.nextInt();
        int[] first = _inpArr(n);
        int[] second = _inpArr(n);
        memo.clear();
        System.out.println(sol(n-1, first, second));
    }

    static int sol(int idx, int[] first, int[] second) {
        ...
        int[][] dp = new int[n + 1][sumFirst + 1];
        for (int i = 0; i <= n; i++) {
            for (int j = 0; j <= sumFirst; j++) {
                dp[i][j] = P_INF;
            }
        }
        dp[0][0] = 0;
        ...
        for (int i = 1; i <= n; i++) {
            for (int j = sumFirst; j >= 0; j--) {
                if (j >= first[i - 1]) {
                    dp[i][j] = min(dp[i][j], dp[i - 1][j - first[i - 1]] + second[i - 1]);
                }
                dp[i][j] = min(dp[i][j], dp[i - 1][j] + first[i - 1]);
            }
        }

        int result = P_INF;
        for (int j = 0; j <= sumFirst; j++) {
            result = min(result, max(j, dp[n][j]));
        }
        return result;
    }

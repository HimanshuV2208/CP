...
public class D {
    ...
    static int sol(int idx, int reqAscLen) {
        int result;
        if (reqAscLen < 0) {
            result = N_INF;
        } else if (idx != 0) {
            if (dp[idx][reqAscLen] == -1) {
                int sol1 = sol(idx - 1, reqAscLen);
                if (freq[idx] <= 0) {
                    dp[idx][reqAscLen] = sol1;
                } else {
                    dp[idx][reqAscLen] = sol(idx - 1, reqAscLen - 1);
                }
                if (!(sol1 == N_INF || sol1 + freq[idx] > reqAscLen)) {
                    if (dp[idx][reqAscLen] == N_INF) {
                        dp[idx][reqAscLen] = sol1 + freq[idx];
                    } else if (sol1 + freq[idx] < dp[idx][reqAscLen]) {
                        dp[idx][reqAscLen] = sol1 + freq[idx];
                    }
                }
            }
            result = dp[idx][reqAscLen];
        } else {
            if (reqAscLen == 0) result = 0;
            else result = N_INF;
        }
        return result;
    }

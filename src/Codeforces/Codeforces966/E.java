...
//        for (int i = 0; i < n; i++) {
//            for (int j = 0; j < m; j++) {
//                ans.add(((long) (min(i + 1, n - k + 1) - max(i - k + 2, 1) + 1) * (min(j + 1, m - k + 1) - max(j - k + 2, 1) + 1)));
//            }
//        }
//        Arrays.sort(a);
//        long res = 0;
...
//        if(true) return;
    ...
    private static void fill(int[][] contri, int i, int j, int k) {
        for (int r = 0; r < k; r++) {
            for (int c = 0; c < k; c++) {
                contri[i + r][j + c]++;
            }
    ...
    static void _printArrLong(long[] arr) {
        StringBuilder sb = new StringBuilder();

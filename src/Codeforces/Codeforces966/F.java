package Codeforces.Codeforces966;
...
public class F {
    ...
    static final int P_INF = 1_000_000;
    ...
    static void solve() {
        ...
        for (int i = 0; i < n; i++) {
            Map<Integer, Integer> cost = getMap(wid, i, hei);
            for (int j = 0; j < k; j++) {
                Iterator<Map.Entry<Integer, Integer>> iterator = cost.entrySet().iterator();
                while (true) {
                    if (!iterator.hasNext()) break;
                    Map.Entry<Integer, Integer> entry = iterator.next();
                    if (j + entry.getKey() <= k) {
                        dp[i + 1][j + entry.getKey()] = min(dp[i + 1][j + entry.getKey()], dp[i][j] + entry.getValue());
                    }
                }
            }
        }
        System.out.println(dp[n][k] == P_INF ? -1 : dp[n][k]);
    }

    private static Map<Integer, Integer> getMap(int[] wid, int i, int[] hei) {
        int a = wid[i];
        int b = hei[i];
        int sum = a + b;
        int area = a * b;
        ...
        do {
            if (a <= 0 || b <= 0) break;
            if (a <= b) {
                map.put(map.size(), map.get(map.size() - 1) + a);
            } else {
                int temp = a;
                a = b;
                b = temp;

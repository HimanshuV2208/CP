package Codeforces.EduCodeforces168;
...
public class B {
    ...
    static void solve() {
        ...
        for(int i = 0; i < n; i++){
            blocked[0][i] = r1.charAt(i) == 'x';
            blocked[1][i] = r2.charAt(i) == 'x';
        }
//        _printGrid(blocked);
        int c = 0;
        for(int i = 1; i < n - 1; i++){
            if(!blocked[0][i] && !blocked[1][i]){
                if(blocked[0][i-1] && blocked[0][i + 1] && !blocked[1][i-1] && !blocked[1][i+1]){
                    c++;
                }
                if(!blocked[0][i-1] && !blocked[0][i + 1] && blocked[1][i-1] && blocked[1][i+1]){
                    c++;
    ...
    static void _printArr(boolean[] arr) {
        StringBuilder sb = new StringBuilder();
        for (boolean x : arr) sb.append((x? 1 : 0)).append(" ");
    ...
    static void _printGrid(boolean[][] grid) {

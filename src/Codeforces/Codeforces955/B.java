package Codeforces.Codeforces955;
...
public class B {
    ...
    static void solve() {
        ...
        long y = sc.nextLong();
        long k = sc.nextLong();
        long stepsToNextMultiple = y - (x % y);
        if (k == stepsToNextMultiple) {
            x += k;
            while (x % y == 0) {
                x /= y;
            }
        ...
        while (1 == 1) {
            long requiredSteps = stepsToNextMultiple;
            if (x == 1) {
                break;
            }
            if (k < requiredSteps) {
                ...
                stepsToNextMultiple = y - x % y;
            }
        }
        if (k != 0) {
            System.out.println(k % (y - 1) + 1);
        } else {
            System.out.println(x);
        }
    }

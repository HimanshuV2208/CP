package Codeforces.Codeforces964;
...
public class F {
    ...
    static long[] fact = new long[200002];
    ...
    static void solve() {
        ...
        while (i < n) {
            if (a[i] != 0) {
                ans = (ans + ((ncr((int) (z + pfx[i]), mid) * ncr((int) (pfx[n] - pfx[i + 1]), mid)) % MOD)) % MOD;
            }
            ++i;
        }
    ...
    static long ncr(int n, int r) {
        ...
        long value = (fact[r] * fact[n - r]) % MOD;
        return (fact[n] * _power(value, MOD - 2, MOD)) % MOD;
    }

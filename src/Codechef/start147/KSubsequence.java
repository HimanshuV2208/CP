...
public class KSubsequence {
    ...
    static void solve() {
        ...
        System.out.println(sol(0, 0, k, a));
    }
    static long sol(int idx, int picks, int k, int[] a) {
        if(idx == a.length && picks != k) return N_INF;
        if(picks == k) return 0;
        int temp = picks;
        if(picks > 0 && picks <= k - 1) temp = 1;
        else if(picks >= k) temp = 2;
        if(dp[idx][temp] != -1) return dp[idx][picks];
        int factor = 2;
        if(picks == 0 || picks == k - 1) factor = 1;
        long take = (long) a[idx] * factor + sol(idx + 1, picks + 1, k, a);
        long noTake = sol(idx + 1, picks, k, a);
        return dp[idx][temp] = max(take, noTake);
    }

...
public class NonPrimes {
    ...
    static boolean[] isPrime = new boolean[201];

    public static void main(String[] args) {
        isPrime = _sieveOfEratosthenes(201);
        int t = sc.nextInt();
        while (t-- > 0) {
            solve();
        }
...
//        if(p.size() >= 3 || (p.size() == 2 && !two)) {
//            System.out.println(p.get(p.size() - 1) + " " + p.get(p.size() - 2));
//            return;
//        }
//        if(np.size() >= 3 || (np.size() == 2 && !one)) {
//            System.out.println(np.get(np.size() - 1) + " " + np.get(np.size() - 2));
//            return;
//        }
        ...
        for (int num : a) {
            for (int prev : seen) {
                if (!isPrime[prev + num]) {
                    int idxNum = map.get(num).get(0);
                    int idxPrev = map.get(prev).get(0);
                    if(idxNum == idxPrev) {
                        if(map.get(prev).size() == 1) continue;
                        idxPrev = map.get(prev).get(1);
                    }
                    System.out.println((idxNum + 1) + " " + (idxPrev + 1));
                    return;
                }
            }
            seen.add(num);

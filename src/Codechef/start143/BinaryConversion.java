...
public class BinaryConversion {
    ...
    static void solve() {
        ...
        int oneCount1 = 0, zeroCount1 = 0;
        for (int i = 0; i < n; i++) {
            if (s.charAt(i) == '0') zeroCount1++;
            else oneCount1++;
        }
        int oneCount2 = 0, zeroCount2 = 0;
        for (int i = 0; i < n; i++) {
            if (t.charAt(i) == '0') zeroCount2++;
            else oneCount2++;
        }
        if (oneCount1 != oneCount2 || zeroCount2 != zeroCount1) {
            nope();
            return;
        }
        int diffs = 0, sum = 0;
        ...
        if ((diffs != 2) || (n != 2) || ((k % 2) != 0)) {
            if (diffs == 2 * sum && sum <= k) yeah();
            else nope();
        } else {
            nope();
        }

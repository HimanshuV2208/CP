...
public class TrickOrTreat {
    ...
    static void solve() {
        ...
        int expectedChildren = sc.nextInt();
        int[] candies = _inpArr(boxes);
        int[] chocolates = _inpArr(boxes);
        Map<Integer, Integer> candiesMap = new HashMap<>();
        Map<Integer, Integer> chocolatesMap = new HashMap<>();
        for (int candy : candies){
            candiesMap.put(candy % expectedChildren, candiesMap.getOrDefault(candy % expectedChildren, 0) + 1);
        }
        for (int chocolate : chocolates){
            chocolatesMap.put(chocolate % expectedChildren, chocolatesMap.getOrDefault(chocolate % expectedChildren, 0) + 1);
        }
        long count = 0;
        for(Map.Entry<Integer, Integer> entry : candiesMap.entrySet()){
            int find = (expectedChildren - entry.getKey()) % expectedChildren;
            if(chocolatesMap.containsKey(find)) count += (long)entry.getValue() * chocolatesMap.get(find);
        }

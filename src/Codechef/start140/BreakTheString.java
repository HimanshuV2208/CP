...
public class BreakTheString {
    ...
    static void solve() {
        ...
        List<Integer> startR = getList(new StringBuilder(s).reverse().toString(), n, false);
        List<Integer> startP = getList(s, n, true);
        Set<Integer> set = new HashSet<>(startR);
        int counter = 0;
        for (int i : startP) {
            int size = n - 2 * i;
            size >>= 1;
            if (set.contains(n - size - 1)) {
                counter++;
            }
        }
    ...
    private static List<Integer> getList(String s, int n, boolean flag) {
        int[] pi = prefixFunction(s);
        List<Integer> startP = new ArrayList<>();
        for (int i = 0; i < n; i++) {
            if (i <= pi[i]) {
                if(flag) startP.add(i);
                else startP.add(n - i - 1);
            }
        }
        return startP;
    }

...
public class Problem1B {
    ...
    static void solve() {
        ...
        //format1 -> R**C**
        //format2 -> AB**
        if(inp.charAt(0) == 'R' && Character.isDigit(inp.charAt(1)) && inp.indexOf('C') != -1){
            System.out.println(for1To2(inp));
        } else {
            System.out.println(for2To1(inp));
        }
    }
    static String for1To2(String s){
        int rPos = s.indexOf('C');
        int row = Integer.parseInt(s.substring(1, rPos));
        int col = Integer.parseInt(s.substring(rPos + 1));
    ...
    static String for2To1(String s){
        int n = s.length();
        int numPos = -1;
        for(int i = n - 1; i >= 0; i--){
        ...
        int row = Integer.parseInt(s.substring(numPos));
        String col = s.substring(0, numPos);
        int colNum = 0;
        for(int i = 0; i < col.length(); i++){
            colNum = colNum *26 + (s.charAt(i) - 'A'+1);
        }
        StringBuilder sb = new StringBuilder();
        sb.append('R').append(row).append('C').append(colNum);
        return sb.toString();
    }

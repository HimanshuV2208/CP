...
public class _01_03_Problem478D {
    ...
    // Height of pyramid is 'h' => blocks needed = 1 + 2 +...h => h(h+1)/2
    // Therefore, h(h+1)/2 <= r+g

    // S1 -> State : dp[i][j] -> numbers of ways to have height 'i' and 'j' red blocks
    // S2 -> Base Cases : dp[0][0] = 1, dp[1][0] = 1, dp[1][1] = 1;
    // S3 -> Transitions : dp[i][j] = ?
    // if last row being added is green : dp[i-1][j]
    // if last row being added is red : dp[i-1][j-i]
    // dp[i][j] = dp[i - 1][j] + dp[i - 1][j - i];

    // Once table is filled
    // Accumulate the entries that lie within permissible range
    ...
    static void solve() {
        ...
        maxH--;
        int blocks = (maxH * (maxH + 1)) / 2;
        // MLE
        // long[][] dp = new long[maxH + 1][r + 1];
        // dp[0][0] = 1;
        // dp[1][0] = 1;
        // dp[1][1] = 1;
        int[] dp = new int[r + 1];
        ...
        for (int height = 1; height <= maxH; height++) {
            //reversed loop to avoid overwriting issues
            for (int red = r; red > -1; red--) {
                if (red >= height) {
        ...
        long ans = 0;
        for(int i = 0; i <= r; i++) {
            if(blocks - i <= g) ans = (ans + dp[i]) % MOD;
        }

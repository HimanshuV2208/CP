...
public class _01_04_Problem839D {
    ...
    static void solve() {
        int n = sc.nextInt();
        int[] a = _inpArr(n);
        int[] multipleCount = new int[N];
        for (int x : a) {
            for(int factor = 1; factor * factor <= x; factor++){
                if(x % factor == 0) {
                    multipleCount[factor]++;
                    if(factor * factor != x) multipleCount[x / factor]++;
                }
            }
        }
        long[] dp = new long[N];
        for(int i = 0; i < N; i++) {
            if(multipleCount[i] > 0) dp[i] = (multipleCount[i] * pow2[multipleCount[i] - 1]) % MOD;
        }
        long ans = 0;
        for(int i = N - 1; i > 1; i--) {

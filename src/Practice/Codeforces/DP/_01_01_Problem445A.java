...
public class _01_01_Problem445A {
    ...
    //S1 -> Defining the sub-problem (state):
    //dp[i] : answer when we only consider elements in [1, i]
    //ignoring elements (i, 100_000]

    //S2 -> Determine base case :
    //dp[1] = cnt[1]
    //dp[0] = 0

    //S3 -> Find the transitions :
    // dp[i] = ?
    // we can take i -> cnt[i] * i + dp[i - 2]
    // we can not take i -> cnt[i - 1]
    ...
    static void solve() {
        ...
        System.out.println(dp[MAX_VAL - 1]);
    }

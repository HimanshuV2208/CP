package Practice.Codeforces.DP;
...
public class _01_02_Problem977F {
    ...
    // !!! We have a better approach than this
    // S1 -> State
    // dp[i] -> length of the longest consecutive subsequence ending at index i
    // S2 -> Base Case
    // dp[0] -> 1
    // S3 -> Transitions
    // dp[i] = ?
    // take and increase -> if(a[i] - a[i] == 1) -> possible dp[i-1] + 1 else 1
    // not take 1

    // a[i] is max 10pow9
    // But let us consider for now it is a[i] <= 200_000
    // S1 -> State
    // dp[a[i]] -> length of longest consecutive subsequence ending at integer a[i]
    // S2 -> Base Case
    // All dp[a[i]] initially 0
    // S3 -> Transitions
    // dp[a[i]] = ?
    // do nothing dp[i]
    // increase length with dp[a[i] - 1]
    // S4 -> Final ans
    // dp[a[i]] = max(dp[a[i]], dp[a[i] - 1] + 1)
    // Another optimization greedily is that it's always best to choose the rightmost a[i]
    // therefore, we can ignore the dp[a[i]] part, dp[a[i]] is always dp[a[i] - 1] + 1
    // I have kept the initial equation for clarity

    // To address a[i] max 10^9, instead of array, we can use a map
    // This works because at max, n = 2*10pow5 => at max 'n' unique keys are present
    ...
    static void solve() {
        ...
        int len = 0;
        for (int i = 0; i < n; i++) {
            dp.put(a[i], max(dp.getOrDefault(a[i], 0),
                            dp.getOrDefault(a[i] - 1, 0) + 1));
            if(dp.get(a[i]) > len) {
                endsAt = a[i];
                len = dp.get(a[i]);
            }

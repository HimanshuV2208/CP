...
public class Problem1872E {
    ...
    static void solve() {
        ...
        for (int i = 0; i < q; i++) {
            int type = sc.nextInt();
            if (type == 1) {
                queries.add(new int[]{1, sc.nextInt() - 1, sc.nextInt() - 1});
            } else {
                queries.add(new int[]{2, sc.nextInt()});
            }
        }
        ...
        for (int[] que : queries) {
            if (que[0] == 1) {
                int change = segTree.query(0, 0, n - 1, que[1], que[2]);
                zeroXor ^= change;
                oneXor ^= change;
            } else {
                if (que[1] == 0) sb.append(zeroXor);
                else sb.append(oneXor);
                sb.append(" ");
            }
        }
